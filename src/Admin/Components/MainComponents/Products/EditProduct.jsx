...
const EditProduct = () => {
  ...
  useEffect(() => {
    if (initialProducts) {
      ...
      if (Array.isArray(initialProducts.category)) {
        setEditProdCategory(
          initialProducts.category.map((cat) => cat._id || cat)
        );
      } else {
        setEditProdCategory(
          initialProducts.category?._id ? [initialProducts.category._id] : []
        );
      }
      ...
      if (Array.isArray(initialProducts.subcategory)) {
        setEditProdSubCategory(
          initialProducts.subcategory.map((sub) => sub._id || sub)
        );
      } else {
        setEditProdSubCategory(
          initialProducts.subcategory?._id
            ? [initialProducts.subcategory._id]
            : []
        );
      }
      ...
      setSelectedSizeChartRefs(
        initialProducts.sizeChartRefs?.map((id) => id.toString()) || []
      );
    }
  }, [initialProducts]);
  ...
  useEffect(() => {
    if (editProdCategory.length > 0) {
      const filtered = subCategories.filter((subcategory) =>
        editProdCategory.includes(subcategory?.category?._id)
      );
  ...
  const handleEditProductSubmit = async (e) => {
    ...
    try {
      ...
      const calculatedOfferPrice =
        editProdActualPrice - editProdActualPrice * (editProdDiscount / 100);
      ...
      editproductFormData.append(
        'isFeaturedProduct',
        editProdCheckboxes.featured
      );
      editproductFormData.append(
        'freeDelivery',
        editProdCheckboxes.freeDelivery
      );
      ...
      editproductFormData.append(
        'sizeChartRefs',
        JSON.stringify(selectedSizeChartRefs)
      );
  ...
  const handleDeleteSizeField = (colorIndex, sizeIndex) => {
    ...
    // Ensure 'sizes' exists
    if (
      updatedFields[colorIndex]?.sizes &&
      updatedFields[colorIndex].sizes.length > 1
    ) {
      updatedFields[colorIndex].sizes = updatedFields[colorIndex].sizes.filter(
        (_, i) => i !== sizeIndex
      );
    } else {
  ...
  const handleDeleteImg = async (image) => {
    if (editProdImage.length === 1) {
      toast.error(
        'At least one image must be present. Cannot delete the last image.'
      );
      return;
    }
    try {
      setEditProdImage((prevImages) =>
  ...
  return (
    <>
      ...
      <form
        action=''
        className='grid lg:grid-cols-2 gap-10 mt-5'
        onSubmit={handleEditProductSubmit}>
        <div className='bg-white rounded-xl shadow-md h-fit'>
          <div className='p-5'>
            <h2 className='text-xl font-medium mb-3 lg:mb-0 text-secondary'>
              Product Information
            </h2>
          </div>
          <hr />
        ...
        bg-gray-100/50 p-2 rounded-md placeholder:text-sm placeholder:font-light placeholder:text-gray-500
         focus:outline-none placeholder:capitalize'
            ...
            <div className='flex items-center justify-between'>
              <Checkbox
                label={
                  <Typography className='font-custom text-secondary text-base font-normal'>
                    Latest Products
                  </Typography>
                }
                color='pink'
        ...
        <div className='bg-white rounded-xl shadow-md p-5 space-y-6 h-fit'>
                  ...
                  <div className='flex items-center gap-2 w-full'>
                    ...
                    <div className='w-full'>
                      <input
                        ...
                        className={`w-full p-2 text-center bg-gray-100/50 border rounded-md text-sm uppercase placeholder:capitalize focus:outline-none ${getContrastYIQ(
                          field.color
                        )}`}
                    ...
                    field.sizes.map((sizeField, sizeIndex) => (
                      <div
                        ...
                        <HiMiniXMark
                          className='text-2xl text-black cursor-pointer'
                          onClick={() =>
                            handleDeleteSizeField(colorIndex, sizeIndex)
                          }
                        />
                      </div>
                    ))}
                ...
                <div className='flex flex-col gap-2 mt-4'>
                  ...
                  <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2'>
                    {sizeChartOptions.map((chart) => {
                      const isChecked = selectedSizeChartRefs.includes(
                        chart._id
                      );
                      ...
                      return (
                        <label
                          ...
                          <input
                            ...
                            onChange={() => {
                              if (isChecked) {
                                setSelectedSizeChartRefs((prev) =>
                                  prev.filter((id) => id !== chart._id)
                                );
                              } else {
                                setSelectedSizeChartRefs((prev) => [
                                  ...prev,
          ...
          <div className='flex justify-center items-center !mt-5'>
            <Button
              type='submit'
              className='bg-buttonBg font-custom font-normal tracking-wider'>
              submit product
            </Button>
          </div>
        </div>
